...
router.post('/create-products', upload2.single('productImage'), async (req, res) => {
    try {
        const { basePrice, variant, brandName, seriesName, categoryType, model, dynamicFields, bestSelling, estimatedPrice } = req.body;
        const dynamicFieldsArray = JSON.parse(dynamicFields);
        const productImage = req.file.originalname;
        const newProduct = new ProductModel({ productImage, basePrice, variant, brandName, seriesName, categoryType, model, dynamicFields: dynamicFieldsArray, bestSelling, estimatedPrice });
